...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  * ================== Vivace.MemoryModule ===================================
  *
  * Copyright (c) 2021 tinyBigGAMES LLC - https://tinybiggames.com
  *
  * Misc modifications and enhancements to work better with Vivace Game Toolkit
  *
  * }
interface
...
type
  ...
  TMemoryModule = class
  public
    class function LoadLibrary(Data: Pointer): Pointer;
    class function GetProcAddress(Module: Pointer; const Name: PAnsiChar): Pointer;
    class procedure FreeLibrary(Module: Pointer);
  end;
...
begin
  ...
  begin
    name := string(PAnsiChar(PByte(CodeBase) + importDesc.Name));
    ...
    try
      ...
      SetLength(Module.Names, Module.NumModules + 1);
    except
    ...
    Module.Names[Module.NumModules] := name;
...
{ TMemoryModule }
class function TMemoryModule.LoadLibrary(Data: Pointer): Pointer;
var
...
begin
  ...
  try
    ...
    begin
      ...
      // notify library about attaching to thread
      successfull := DllEntry(HINST(code), DLL_THREAD_ATTACH, nil);
      if not successfull then
      begin
        SetLastError(ERROR_DLL_INIT_FAILED);
        Abort;
    ...
    // cleanup
    TMemoryModule.FreeLibrary(Module);
    Exit;
  end;
end;

class function TMemoryModule.GetProcAddress(Module: Pointer; const Name: PAnsiChar): Pointer;
var
  CodeBase: Pointer;
  Idx: Integer;
  i: DWORD;
...
class procedure TMemoryModule.FreeLibrary(Module: Pointer);
var
  i: Integer;
  DllEntry: TDllEntryProc;
  mmodule: PMemoryModule;
begin
  ...
  begin
    ...
    DllEntry(HINST(mmodule.CodeBase), DLL_THREAD_DETACH, nil);
